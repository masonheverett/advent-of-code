...
export const solve = (data) => {
  ...
  for (index = 0; index < data.length; index++) {
    const [col, row] = data[index].split(',').map(n => parseInt(n))
    delete costs[coordsToId(row, col)]
    if (isBlocked(_.cloneDeep(costs))) break
  }
  console.log(data[index])
}

const isBlocked = (costs) => {
  ...
  while (heap.size > 0) {
    ...
    if (current.row === RANGE - 1 && current.col === RANGE - 1) break
    // Check neighbors
  ...
  return costs[coordsToId(RANGE - 1, RANGE - 1)] === Number.POSITIVE_INFINITY
}
