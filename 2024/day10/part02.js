...
export const solve = (data) => {
  ...
  console.log(_.sum(Object.values(zeroSets).map(set => _.sum(Object.values(set)))))
}
...
const findPaths = (grid, zeroSets, zeroCoords, currCoords, number) => {
  ...
  if (grid[currCoords[0]][currCoords[1]] === 9 && number === 9) {
    if (!zeroSets[coordString(zeroCoords)][coordString(currCoords)]) {
      zeroSets[coordString(zeroCoords)][coordString(currCoords)] = 0
    }
    zeroSets[coordString(zeroCoords)][coordString(currCoords)]++
    return
  }
